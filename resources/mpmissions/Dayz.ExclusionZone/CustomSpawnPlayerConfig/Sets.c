	...
	void StartSetsPlayer(PlayerBase player,int use_set_id)
	{
		...
		switch( use_set_id ) //Раздаем лут (можно использовать значения от 1 и выше, только не 0 и не -1)
		{
			case 1: //Set with number 1 (Сет с номером 1)
			{
				...
				itemCreated = player.GetInventory().CreateInInventory("aod_backpack_loner1_tortilla"); 	// Выдаем рюкзак и записываем в переменную itemCreated
...
//					SetRandomHealthItem(itemCreated);												// Выдаем рюкзаку рандомное качество
					itemCreated1 = itemCreated.GetInventory().CreateInInventory("TacticalBaconCan");			// Добавляем в инвентарь созданного рюкзака яблоко и записываем в переменную itemCreated1
...
//					}
				...
				itemCreated = player.GetInventory().CreateInInventory("aod_pants_loner5_gorka");		// Выдаем игроку штаны
				itemCreated = player.GetInventory().CreateInInventory("aod_body_loner6_gorka");	// Выдаем игроку куртку
				itemCreated = player.GetInventory().CreateInInventory("TacticalGloves_Black");	// Выдаем игроку перчатки
				itemCreated = player.GetInventory().CreateInInventory("RadarCap_Brown");	// Выдаем игроку Шапку
				...
				itemCreated = player.GetInventory().CreateInInventory("Ammo_12gaPellets");	// Выдаем игроку магазины к FNX в любой свободный слот в инвентаре
				itemCreated = player.GetInventory().CreateInInventory("Ammo_12gaPellets");	// Выдаем игроку магазины к FNX в любой свободный слот в инвентаре
				itemCreated = player.GetInventory().CreateInInventory("Ammo_12gaPellets");		// Выдаем игроку магазины к AKM в любой свободный слот в инвентаре
				itemCreated = NULL;															// Обнуляем значение переменной после работы с ней, нужно если мы будем использовать ее далее
				itemCreated = player.GetHumanInventory().CreateInHands("Mp133Shotgun");				// Выдаем игроку AKM в руки
